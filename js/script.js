...
for (let i = 1; i < 9; i++) {
  ...
  chessColumns.map(letter => {
    ...
    div.innerHTML =
      '<div class="item" id="' + squareName + '">' + squareName + "</div>";
    row.appendChild(div);
  });
  ...
  event => {
    ...
    if (event.target.classList.contains("item")) {
      ...
      if (!start.innerText) {
        ...
        document
          .getElementById(squareName)
          .setAttribute("style", "background: #ff0000a3;");
...
getPath.addEventListener("click", async function(event) {
  ...
  if (startValue && endValue) {
    const res = await fetch(
      `https://knight-shortest.herokuapp.com/apiv1/shortestpath?start=${startValue}&end=${endValue}`
    );
    ...
    Object.keys(json).map(square => {
      ...
      if (square != 1) {
        document
          .getElementById(json[square])
          .setAttribute("style", "background: yellow;");
      }
      result.appendChild(div);
    });
    document
      .getElementById(end.innerText)
      .setAttribute("style", "background: #ff0000a3;");
  } else {
